...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class FastZip
	{
			...
			using (zipFile_ = new ZipFile(inputStream)) {
				...
				while (continueRunning_ && enumerator.MoveNext()) {
					...
					if (entry.IsFile)
					{
						...
						if (directoryFilter_.IsMatch(System.IO.Path.GetDirectoryName(entry.Name)) && fileFilter_.IsMatch(entry.Name)) {
							ExtractEntry(entry);
						}
					}
		...
		void ExtractEntry(ZipEntry entry)
		{
			...
			if ( doExtraction ) {
					if ( entry.IsDirectory ) {
						...
						dirName = System.IO.Path.GetDirectoryName(System.IO.Path.GetFullPath(targetName));
					}
			}
		...
		static bool NameIsValid(string name)
		{
			return (name != null) &&
				(name.Length > 0) &&
				(name.IndexOfAny(System.IO.Path.GetInvalidPathChars()) < 0);
		}
#endif
		#endregion
